...
public function getAnnouncements(){
    {
        ...
        $builder->select('announcements.title, announcements.content, announcements.created_at, announcements.updated_at, users.fname, users.lname, users.picture_url');
...
public function getPetCounts()
{
    ...
    $query = $this->db->query('SELECT COUNT(*) AS available, SUM(CASE WHEN status = "processing" THEN 1 ELSE 0 END) AS processing, SUM(CASE WHEN status = "adopted" THEN 1 ELSE 0 END) AS adopted FROM pets');
